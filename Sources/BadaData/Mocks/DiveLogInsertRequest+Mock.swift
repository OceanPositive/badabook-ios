...
extension DiveLogInsertRequest {
    package static var mock: DiveLogInsertRequest {
        DiveLogInsertRequest(
            ...
            gasType: [DiveGasType.air, DiveGasType.nitrox(oxygenPercentage: 100)].randomElement()?.domain,
            equipment: nil,
            maximumDepth: .m(Double.random(in: 10...40)),
            averageDepth: .m(Double.random(in: 5...20)),
            airTemperature: .celsius(Double.random(in: 20...35)),
            ...
            surge: [Surge.strong, Surge.medium, Surge.light].randomElement()?.domain,
            visibility: [Visibility.high, Visibility.average, Visibility.low].randomElement()?.domain,
            visibilityDistance: .m(Double.random(in: 5...30)),
